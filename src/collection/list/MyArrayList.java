...
/**
 * 직접 구현해본 배열 리스트에 추상화한 리스트 인터페이스 상속
 *
 * - 장점
 * 1.동적으로 최대 크기를 변경해준다.
 * 2.인덱스를 통한 조회는 즉시 찾아주어 성능이 좋다.
 *   그래서 마지막에 데이터 추가/삭제 또한 효율적이다.
 *
 * - 단점
 * 1.임의의 크기만큼 늘리므로 메모리 낭비가 있을 수 있다.
 * 2.제일 앞과 중간에 데이터 추가/삭제는 비효율적이다.
 */
public class MyArrayList<E> implements MyList<E>{ // 제네릭 타입 추가
